...
function drawWheel(rot = 0) {
    ...
    // ── POINTER drawn directly on canvas at angle=0 (right side) ──
    // This guarantees perfect alignment with segments — same coordinate system.
    const pTip  = C + R - 4;      // tip is 4px inside rim
    const pBase = C + R + 18;     // base is 18px outside rim
    const pHalf = 13;              // half-height of pointer
    ctx.beginPath();
    ctx.moveTo(pTip,  C);          // tip (pointing left into wheel)
    ctx.lineTo(pBase, C - pHalf);  // top of base
    ctx.lineTo(pBase, C + pHalf);  // bottom of base
    ctx.closePath();
    ctx.fillStyle   = '#2d5016';
    ctx.shadowColor = 'rgba(45,80,22,.5)';
    ctx.shadowBlur  = 6;
    ctx.fill();
    ctx.shadowBlur  = 0;
    // White outline
...
function handleSpin() {
    ...
    .then(data => {
        ...
        // CORRECT formula (mathematically verified):
        // drawWheel segment i midpoint = -PI/2 + rot + i*arc + arc/2
        // For winner midpoint at pointer (angle=0): rot = PI/2 - idx*arc - arc/2
        // Small random jitter ±20% keeps pointer clearly inside segment, never on edge.
        const jitter    = (Math.random() * 0.4 - 0.2) * arc;
        let targetRot   = Math.PI/2 - idx*arc - arc/2 + jitter;
        // Ensure minimum 8 full forward rotations
